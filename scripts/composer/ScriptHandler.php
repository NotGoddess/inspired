...
class ScriptHandler {
  ...
  public static function createRequiredFiles(Event $event) {
    ...
    // Assumption that we put our repos in /srv/www/sitename/
    $siteBaseName = basename( dirname( $root ) );
    // remove subdomain (www) if there and escape periods so they can be used in regex
    $SiteBaseName = str_replace('www.','', $siteBaseName);
    $escSiteBaseName = str_replace('.','\.', $siteBaseName);
    ...
    // Aureon specific
      $content = "
\$config_directories[CONFIG_SYNC_DIRECTORY] = '../config/drupal8/sync';
\$settings['trusted_host_patterns'] = array(
   'webstaging.\.alliancetechnologies\.net',
   '^.+${escSiteBaseName}',
);";
      file_put_contents($root . '/sites/default/settings.php', $content, FILE_APPEND);
      $event->getIO()->write("Please update sites/default/settings.php file trusted_host_patterns with the proper site name if it is not ${siteBaseName}");
    }
